...
const checkModelController = ($scope, baseService,$sce) => {
	$scope.displayed = [];
	$scope.sp = {};
	$scope.tableState = {};
	...
	$scope.details = function (item, detailType) {
		if (item.type == 0) {
			...
			baseService.getJson(baseService.api.checkModel + 'getCheckInfo', {
				...
				baseService.confirmDialog(750, '详情', data, 'tpl/program_details.html', function (type,ngDialog) {
					var status = '';
					if(type == 3){
						status = 1;
					}else{
...
checkModelController.$inject = ['$scope', 'baseService','$sce'];
